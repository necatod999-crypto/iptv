...
function parseChannels(data) {
  ...
  rows.sort((a,b) => a.name.localeCompare(b.name, 'sr'));

  console.log('MOVE exact channels', JSON.stringify({
    content: src.length,
    channels: rows.length,
    sample: rows.slice(0,10).map(x => ({id:x.id,name:x.name,category:x.category}))
  }));
  return rows;
}
...
app.post('/login', async (req,res) => {
  try {
    ...
    const rows = channels.map(c => `<tr><td>${esc(c.name)}</td><td>${esc(c.category)}</td></tr>`).join('');
    res.send(page(`<div class=card><b class=ok>Uspesno.</b><p><b>TV kanali: ${channels.length}</b></p><p><a href='${esc(m3u)}'>Preuzmi pravi M3U</a></p></div><div class=card><table><tr><th>Kanal</th><th>Kategorija</th></tr>${rows}</table></div>`));
  } catch (e) {
...
app.get('/playlist.m3u', async (req,res) => {
  try {
    ...
    res.set('content-disposition','attachment; filename="move-tv.m3u"');
...
function proxyRef(raw, srcUrl, proxyBase) {
  ...
  try {
    if (/^https?:\/\//i.test(v)) {
      const a = new URL(v), s = new URL(srcUrl);
      if (a.origin === s.origin) return `${proxyBase}__root__/${a.pathname.replace(/^\/+/, '')}${a.search}`;
    }
  } catch {}
  return v;
}
...
function rewriteDash(text, srcUrl, proxyBase) {
  ...
  out = out.split(upstreamDir).join(proxyBase);
  ...
  try {
    ...
    if (src.url.includes('.m3u8')) {
      const lines=body.split(/\r?\n/).map(line=>{
        if (!line) return line;
        if (line.startsWith('#')) return line.replace(/URI="([^"]+)"/g,(_m,v)=>`URI="${proxyRef(v,src.url,proxyBase)}"`);
        return proxyRef(line,src.url,proxyBase);
      });
    ...
    console.log('manifest error',id,e.message);
...
app.get('/play/:token/:id/*rest', async (req,res) => {
  ...
  try {
    ...
    const makeUrl = s => {
      let u;
      if (rest.startsWith('__root__/')) u=new URL('/'+rest.slice(9),new URL(s.url).origin);
      else u=new URL(rest,new URL('.',s.url));
      for (const [k,v] of Object.entries(req.query)) u.searchParams.set(k,String(v));
      return u.toString();
    };
    let r=await upstream(src,makeUrl(src),req);
    if (r.status===401 || r.status===403) { src=await sourceFor(creds,id,true); r=await upstream(src,makeUrl(src),req); }
    if (!r.ok && r.status!==206) { console.log('segment error',id,r.status,rest.slice(0,120)); throw new Error(`MOVE segment HTTP ${r.status}`); }
    res.status(r.status);
    for (const h of ['content-type','content-length','content-range','accept-ranges']) { const v=r.headers.get(h); if(v) res.set(h,v); }
...
app.get('/health',(_req,res)=>res.json({ok:true,service:'nova-move-render-fixed',maxStreams:MAX_ACTIVE_STREAMS}));
app.listen(PORT,'0.0.0.0',()=>console.log(`NOVA MOVE fixed listening on ${PORT}`));
